...
import type { FamilyClassification, ProductFamily, ProductSuperfamily } from "@/lib/product-family";
...
async function findLocalAlternatives(
  ...
  for (const { product } of pool.values()) {
    ...
    eligible.push({ product, family: candFamily, compat });
  }
  ...
  for (const candidate of deduped) {
    const { product, compat } = candidate;
    ...
    const dataConfidence = calculateDataConfidence(candidateNutrition, candidateIngs, compat.affinity / 100);
    ...
    const scoring = calculateAlternativeScore(
      {
        ...
        dataConfidence,
        familyAffinity: compat.affinity,
  ...
  for (const candidate of shortlist) {
    const { product, scoring, dataConfidence } = candidate;
    ...
    // Only label "better" when there is evidence; otherwise it is a comparable
    // product in the same family with a neutral framing.
    ...
    results.push({
      ...
      recommendationScore: scoring.overallScore,
      reasons: scoring.reasons,
      whyBetter: evidence
        ? scoring.reasons[0]?.detail ?? "Similar alternative"
