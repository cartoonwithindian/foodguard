...
import { scoreNutrition, scoreIngredient, computeFoodGuardScore, scoreToAssessment } from "@/lib/scoring";
...
export async function runAnalysis(input: AnalyzeInput): Promise<{ frontend: FrontendAnalysisResult; meta: AnalyzeMeta }> {
  ...
  ingredientAnalysis.items.forEach((item, index) => {
    if (item.matched) factors.push(scoreIngredient(item, index, ingredientAnalysis.items.length));
  });
  const nutritionScoring = scoreNutrition(productNutrition);
  factors.push(...nutritionScoring.factors);
  const initialFoodGuardScore = computeFoodGuardScore(ingredientAnalysis.items, productNutrition);
  let score = initialFoodGuardScore.score;
  let scoreLabel = initialFoodGuardScore.label;
  let scoreBreakdown = initialFoodGuardScore.breakdown;

  // ── 8. Confidence (evidence-derived, not random) ──
  const productConfidence = product ? product.productDataConfidence : 0.7;
  const nutritionConfidence = nutritionScoring.confidence;
  const dataCompleteness = ingredientsText ? 1 : 0.3;
  let confidence = Math.round(
    (0.35 * productConfidence + 0.25 * nutritionConfidence + 0.3 * ingredientAnalysis.matchRate + 0.1 * dataCompleteness) * 100,
  ) / 100;
  let assessment = scoreToAssessment(score, confidence);
  ...
  // ── 10. FSSAI Regulatory Analysis ──
  let fssaiResult: FSSAIAnalysisResult | null = null;
  try {
    const fssaiAnalyzer = new FSSAIAnalyzer();
    fssaiResult = await fssaiAnalyzer.analyze({
      product: product ? { name: product.name, category: product.category, barcode: product.barcode } : undefined,
      ingredients: [...new Map(
        ingredientAnalysis.items.filter(i => i.matched).map(i => [i.name.toLowerCase(), i.name]),
      ).values()],
      nutrition: productNutrition,
      labelData: {
        hasIngredientsList: !!ingredientsText,
        hasNutritionInfo: !!productNutrition,
      },
      // The physical label is never verified by a barcode/OCR scan; the
      // labelling check therefore reports INSUFFICIENT_DATA instead of
      // asserting the label passes or fails.
      labelVerified: false,
      category: product?.category,
    });
    
    if (fssaiResult.warnings.length > 0) {
      warnings.push(...fssaiResult.warnings);
    }
  } catch (error) {
    logger.warn("fssai_analysis_failed", { error: String(error) });
  }

  const regulatoryFindingCount = fssaiResult?.regulatoryCheckDetails
    ? Object.values(fssaiResult.regulatoryCheckDetails).reduce((count, check) => count + check.findings.length, 0)
    : 0;
  const foodGuardScore = computeFoodGuardScore(ingredientAnalysis.items, productNutrition, regulatoryFindingCount);
  score = foodGuardScore.score;
  scoreLabel = foodGuardScore.label;
  scoreBreakdown = foodGuardScore.breakdown;
  assessment = scoreToAssessment(score, confidence);
  ...
  // ── 12. AI Analysis Explanation ──
  let aiAnalysis: AIAnalysisExplanation | null = null;
  try {
    const aiProvider = getAIProvider();
    if (aiProvider.explainAnalysis) {
      ...
      const aiInput = {
        ...
        nutrition: productNutrition ? {
          ...
          sugar: productNutrition.nutrients.sugars?.value,
        ...
        regulatory: fssaiResult ? [{
          type: "FSSAI",
          status: fssaiResult.overallStatus,
          details: fssaiResult.warnings.join("; "),
        }] : [],
        chemicalInfo: [],
        webEvidence: webResearchResults.map(e => ({
          title: e.title,
          url: e.url,
          snippet: e.snippet,
      ...
      aiAnalysis = await aiProvider.explainAnalysis(aiInput);

      // Use AI confidence to adjust overall confidence
      if (aiAnalysis.confidence > 0) {
        confidence = Math.round((confidence * 0.7 + aiAnalysis.confidence * 0.3) * 100) / 100;
      }
    }
  } catch (error) {
    logger.warn("ai_analysis_failed", { error: String(error) });
  }
